                      ...
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div>
                          ...
                          {room.gender_preference && (
                            <div className="text-xs text-gray-400 mt-0.5">👥 {room.gender_preference === 'any' ? 'Everyone' : room.gender_preference === 'women' ? 'Women Only' : 'Men Only'}</div>
                          )}
                          <div className="text-xs text-gray-500 mt-1">Created by user ID: {room.created_by?.slice(0, 8)}...</div>
                        </div>
                      </div>
                ...
                <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
                  <div className="bg-gray-900 rounded-2xl border border-gray-700 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                    <div className="flex items-center justify-between p-5 border-b border-gray-800">
                      ...
                      <button onClick={() => setShowAddRoom(false)} className="text-gray-500 hover:text-white text-xl cursor-pointer">×</button>
                    </div>
                    <div className="p-5 flex flex-col gap-4">
                      ...
                      <div>
                        ...
                        <select value={newRoom.gradient} onChange={(e) => setNewRoom({...newRoom, gradient: e.target.value})} className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-green-500">
              ...
              <div className="flex flex-col gap-3">
                {rooms.length === 0 ? (
                  <div className="bg-gray-900 rounded-2xl border border-gray-800 p-8 text-center">
                    ...
                    <div className="text-sm text-gray-500">Approve rooms from the Approvals tab or add one manually.</div>
                  </div>
                ) : rooms.map((room) => {
                  const pct = Math.round((room.seats_filled / room.seats_total) * 100)
                  return (
                    <div key={room.id} className="bg-gray-900 rounded-2xl border border-gray-800 p-4 md:p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <span className={`text-xs font-bold px-2 py-1 rounded-lg flex-shrink-0 ${pct >= 80 ? 'bg-red-900 text-red-400' : 'bg-green-900 text-green-400'}`}>
                          {pct}% full
                        </span>
                      </div>
