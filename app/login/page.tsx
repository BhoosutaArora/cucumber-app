...
export default function Login() {
  ...
  function handleSignUpClick() {
    if (!email || !password) { setMessage('Please enter your email and password'); setMessageType('error'); return }
    ...
    <main className="min-h-screen bg-green-50 flex items-center justify-center px-4 py-8">
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-green-100 w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 md:px-8 py-6 md:py-8">
          <h2 className="text-xl md:text-2xl font-extrabold text-gray-900 mb-1">{isSignUp ? 'Create your account' : 'Sign in to Cucumber'}</h2>
          <p className="text-sm text-gray-400 mb-5 md:mb-6">{isSignUp ? 'Start your travel journey today' : 'Your tribe is waiting for you'}</p>
          ...
          <div className="mb-3">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 transition-all" />
          </div>

          <div className="mb-5">
            <label className="block text-xs font-bold text-gray-500 mb-1.5">Password</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 6 characters" onKeyDown={(e) => { if (e.key === 'Enter') { isSignUp ? handleSignUpClick() : handleSignIn() } }} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 transition-all" />
          </div>
          ...
          <button onClick={isSignUp ? handleSignUpClick : handleSignIn} disabled={loading} className="w-full py-3 md:py-3.5 rounded-xl bg-gradient-to-r from-green-400 to-green-500 text-white font-bold text-sm hover:shadow-lg hover:shadow-green-200 hover:scale-[1.02] transition-all disabled:opacity-50 cursor-pointer">
