...
export default function Home() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Fetch rooms for "Travel here" button
  ...
  function getRoomForDestination(location: string) {
    ...
    return rooms.find(r => r.destination?.toLowerCase().includes(lower) || lower.includes(r.destination?.toLowerCase()))
  }
  ...
  return (
    <main className="min-h-screen bg-white font-sans">
      ...
      <section className="w-full flex flex-col items-center justify-center relative overflow-hidden px-5 md:px-8 pt-24 pb-10">
        ...
        <div className="relative z-10 text-center max-w-3xl w-full">
          ...
          <p className="text-sm md:text-lg text-gray-500 leading-relaxed max-w-xl mx-auto mb-6">
            See real trips. Feel inspired. Join a room going there. Instagram shows you Shimla. Cucumber takes you there.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <a href="/rooms" className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-gradient-to-r from-green-400 to-green-500 text-white font-extrabold text-sm shadow-lg shadow-green-200 hover:shadow-xl hover:scale-105 transition-all text-center">
              Browse Trips →
            </a>
            {!username && (
      ...
      {/* ── SHARE A TRAVEL MEMORY BUTTON ── */}
      <section className="w-full px-4 md:px-0 flex justify-center pb-4 sticky top-14 md:top-16 z-40 bg-white border-b border-green-50 py-3">
        <button
      ...
      <section className="w-full max-w-xl mx-auto px-4 py-6 flex flex-col gap-6">
        ...
        {posts.map((post: any) => {
          ...
          const timeAgo = post.created_at ? new Date(post.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : ''
            ...
            <div className="p-5 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
              ...
              {postSuccess ? (
                ...
                <>
                  ...
                  >
                    {postImagePreview ? (
                      <img src={postImagePreview} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  {/* Vibe */}
                  <div>
                    <div className="text-xs font-semibold text-gray-500 mb-2">What's the vibe?</div>
