...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white font-sans">
      ...
      {/* ── HERO ── */}
      <section className="min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden px-5 md:px-8" style={{paddingTop: '80px', paddingBottom: '60px'}}>
        ...
        <div className="relative z-10 text-center max-w-5xl w-full">
          <div className="inline-flex items-center gap-2 bg-white border border-green-200 rounded-full px-3 md:px-4 py-1.5 md:py-2 text-xs font-bold text-green-700 mb-6 md:mb-8 shadow-sm">
            ...
            🚀 {rooms.length} trip{rooms.length !== 1 ? 's' : ''} launching now — limited seats!
          </div>
          ...
          <p className="text-sm md:text-lg text-gray-500 leading-relaxed max-w-xl mx-auto mb-8 md:mb-10">
            Cucumber connects you to small, curated travel rooms of 5–8 people who share your exact travel vibe. No boring group tours. No lonely solo trips.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-16">
            {[
              { num: 'Weekend', label: 'Short trips' },
              { num: '₹2,999+', label: 'Starting price' },
              { num: '5–8', label: 'People per room' },
              { num: `${rooms.length} open`, label: 'Trips right now' },
            ].map((s) => (
      ...
      <section className="w-full bg-green-50 py-14 md:py-24 px-4 md:px-16">
        <div className="text-center mb-10 md:mb-14">
          ...
          <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight text-gray-900 mb-3">Pick your trip. Find your tribe.</h2>
          <p className="text-sm md:text-base text-gray-500">Real trips. Real people. Limited seats.</p>
        </div>
            ...
            return (
              <div key={room.id} className="bg-white rounded-2xl overflow-hidden border border-green-100 hover:-translate-y-1 hover:shadow-xl transition-all duration-300">
                <div className="h-48 relative flex items-end p-4 overflow-hidden">
                  ...
                  {isAlreadyMember && (
                    <span className="absolute top-3 right-3 text-xs font-bold bg-green-500 text-white px-2 py-1 rounded-full z-10">✅ Joined</span>
                  )}
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <div className="text-lg font-extrabold text-green-700">₹{room.price?.toLocaleString()}</div>
                    </div>
                  ...
                  {isAlreadyMember ? (
                    <a href={'/rooms/' + room.id + '/room'} className="block w-full py-3 rounded-xl bg-gradient-to-r from-green-400 to-green-500 text-white font-bold text-sm text-center hover:scale-105 transition-transform cursor-pointer">
                      Enter Room →
                    </a>
                  ) : (
                    <a href={'/rooms/' + room.id} className="block w-full py-3 rounded-xl bg-gradient-to-r from-green-400 to-green-500 text-white font-bold text-sm text-center hover:scale-105 transition-transform cursor-pointer">
                      View Room & Join →
                    </a>
                  )}
                </div>
      ...
      <footer className="w-full bg-gray-950 py-10 md:py-16 px-4 md:px-16">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between border-t border-gray-800 pt-6 gap-3">
            <span className="text-xs text-gray-600 text-center">© 2026 Cucumber Travel · Made with 🥒 in India</span>
