...
export default function RoomPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setUser(user)
      const { data: membership } = await supabase.from('room_members').select('id').eq('room_id', id).eq('user_id', user.id).single()
      if (!membership) { window.location.href = '/rooms/' + id; return }
      const { data: roomData } = await supabase.from('Rooms').select('*').eq('id', id).single()
      setRoom(roomData)
      const { data: membersRaw } = await supabase.from('room_members').select('*').eq('room_id', id)
      const membersWithProfiles = await Promise.all((membersRaw || []).map(async (member) => {
  ...
  return (
    <main className="min-h-screen bg-green-50 font-sans">
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-12 h-14 bg-white border-b border-green-100 shadow-sm">
        <a href="/" className="text-xl font-extrabold text-green-700">cucumber<span className="text-green-400">.</span></a>
        <a href="/rooms" className="text-sm font-semibold text-gray-500 hover:text-green-700">Back to Rooms</a>
      </nav>
      <div className="pt-20 pb-16 px-4 md:px-16 max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-green-100 p-5 mb-4">
          ...
          {members.length === 0 ? (
            ...
            <div className="space-y-2">
              {members.map((member) => (
                <div key={member.id} onClick={() => { window.location.href = '/profile/' + member.username }} className="flex items-center gap-3 hover:bg-green-50 rounded-xl p-2 transition-all cursor-pointer">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div onClick={() => { window.location.href = '/chat' }} className="block py-4 rounded-2xl bg-white border border-green-200 text-green-700 font-bold text-sm text-center hover:bg-green-50 transition-all cursor-pointer">Group Chat</div>
          <div onClick={() => { window.location.href = '/video-call' }} className="block py-4 rounded-2xl bg-gradient-to-r from-green-400 to-green-500 text-white font-bold text-sm text-center hover:shadow-lg transition-all cursor-pointer">Video Call</div>
        </div>
        <div className="bg-white rounded-2xl border border-green-100 p-5 mb-4">
